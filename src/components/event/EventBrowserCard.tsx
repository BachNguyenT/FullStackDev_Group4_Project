...
function EventBrowserCard({
  ...
  async function sendJoinRequest() {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/send-join-request?${queryParams.toString()}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="p-[10px] min-w-[250px] max-w-[320px] rounded-md overflow-hidden shadow-lg bg-white">
      ...
      {eventStatus === "Completed" && (
        <Button
          className="w-full h-[30px] rounded-sm bg-gray-400 text-white font-semibold text-sm"
          variant="ghost"
          animated={false}
        >
          Event Completed
